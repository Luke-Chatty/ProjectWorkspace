<#
.Synopsis
   PWS Script for Development Projects
.DESCRIPTION
   This script will capture user input, create an AD group in the correct OU, take a copy of the folder structure template and apply correct permissions of newly created AD group using iCACLs.
.NOTES
  Version:        1.4
  Author:         Luke Chatburn
  Creation Date:  30/09/19
  Purpose/Change: 
Modified all anchors to allow for different resolutions. Size is no-longer fixed and if the default window is warped user can resize accordingly.
Fixed clipping issue with Development project label.
Fixed permission issue where users could save outside folder structure despite not being able to see what they save.
Set default state of checkbox to checked, 90% of the time an AD group will be created. (previously unchecked)

#>


Add-Type -AssemblyName System.Windows.Forms
[System.Windows.Forms.Application]::EnableVisualStyles()

$WinForm                         = New-Object system.Windows.Forms.Form
$WinForm.ClientSize              = '410,350'
$WinForm.text                    = "PWS - New Development Project"
$WinForm.TopMost                 = $false
$WinForm.BackColor               = "#f7fcff"


#Icon
$iconBase64                      = ''
$iconBytes                       = [Convert]::FromBase64String($iconBase64)
$stream                          = New-Object IO.MemoryStream($iconBytes, 0, $iconBytes.Length)
$stream.Write($iconBytes, 0, $iconBytes.Length);
$iconImage                       = [System.Drawing.Image]::FromStream($stream, $true)
$WinForm.Icon                    = [System.Drawing.Icon]::FromHandle((New-Object System.Drawing.Bitmap -Argument $stream).GetHIcon())

#SeddonLogo
$base64ImageString = ""
$imageBytes = [Convert]::FromBase64String($base64ImageString)
$ms = New-Object IO.MemoryStream($imageBytes, 0, $imageBytes.Length)
$ms.Write($imageBytes, 0, $imageBytes.Length);
$Seddonlogo = [System.Drawing.Image]::FromStream($ms, $true)

#SeddonLogoPictureBox
$pictureBox = new-object Windows.Forms.PictureBox
$pictureBox.Width =  224
$pictureBox.Height =  65 
$pictureBox.Location = New-Object System.Drawing.Size(112,20) 
$pictureBox.Image = $Seddonlogo;



$PWSNameText                     = New-Object system.Windows.Forms.TextBox
$PWSNameText.multiline           = $false
$PWSNameText.text                = "Contract Name"
$PWSNameText.width               = 152
$PWSNameText.height              = 20
$PWSNameText.Anchor              = 'top,right'
$PWSNameText.location            = New-Object System.Drawing.Point(156,177)
$PWSNameText.Font                = 'Segoe UI,10'
$PWSNameText.ForeColor           = "#000000"

$PWSID                           = New-Object system.Windows.Forms.Label
$PWSID.text                      = "Contract Number:"
$PWSID.AutoSize                  = $true
$PWSID.width                     = 25
$PWSID.height                    = 20
$PWSID.Anchor                    = 'top,left'
$PWSID.location                  = New-Object System.Drawing.Point(32,149)
$PWSID.Font                      = 'Segoe UI,10'

$CreateButton                    = New-Object system.Windows.Forms.Button
$CreateButton.text               = "Create PWS Folder"
$CreateButton.width              = 146
$CreateButton.height             = 30
$CreateButton.Anchor             = 'top,left'
$CreateButton.location           = New-Object System.Drawing.Point(136,286)
$CreateButton.Font               = 'Segoe UI,10'

$PWSName                         = New-Object system.Windows.Forms.Label
$PWSName.text                    = "Contract Name:"
$PWSName.AutoSize                = $true
$PWSName.width                   = 25
$PWSName.height                  = 10
$PWSName.Anchor                  = 'top,left'
$PWSName.location                = New-Object System.Drawing.Point(32,179)
$PWSName.Font                    = 'Segoe UI,10'

$PWSIDText                       = New-Object system.Windows.Forms.TextBox
$PWSIDText.multiline             = $false
$PWSIDText.text                  = "L19XXX"
$PWSIDText.width                 = 152
$PWSIDText.height                = 20
$PWSIDText.Anchor                = 'top,right'
$PWSIDText.location              = New-Object System.Drawing.Point(156,147)
$PWSIDText.Font                  = 'Segoe UI,10'

$Label2                          = New-Object system.Windows.Forms.Label
$Label2.text                     = "Fill out the following form to create your new Project Workspace"
$Label2.AutoSize                 = $true
$Label2.width                    = 25
$Label2.height                   = 10
$Label2.Anchor                   = 'top,left'
$Label2.location                 = New-Object System.Drawing.Point(13,93)
$Label2.Font                     = 'Segoe UI,10'

$Label3                          = New-Object system.Windows.Forms.Label
$Label3.text                     = "Development Project Folder."
$Label3.AutoSize                 = $true
$Label3.width                    = 25
$Label3.height                   = 10
$Label3.Anchor                   = 'top,left'
$Label3.location                 = New-Object System.Drawing.Point(111,114)
$Label3.Font                     = 'Segoe UI,10,style=Bold,Underline'
$Label3.ForeColor                = "#022943"

$ToolTip1                        = New-Object system.Windows.Forms.ToolTip
$ToolTip1.isBalloon              = $false

$ToolTip2                        = New-Object system.Windows.Forms.ToolTip
$ToolTip2.isBalloon              = $false


$ADGroup                         = New-Object system.Windows.Forms.CheckBox
$ADGroup.AutoSize                = $false
$ADGroup.width                   = 100
$ADGroup.height                  = 20
$ADGroup.location                = New-Object System.Drawing.Point(156,209)
$ADGroup.Font                    = 'Microsoft Sans Serif,10'
$ADGroup.Anchor                  = 'top,right'
$ADGroup.Checked                 = $true

$Label5                          = New-Object system.Windows.Forms.Label
$Label5.text                     = "Create AD Group?"
$Label5.AutoSize                 = $true
$Label5.width                    = 25
$Label5.height                   = 10
$Label5.Anchor                   = 'top,left'
$Label5.location                 = New-Object System.Drawing.Point(32,209)
$Label5.Font                     = 'Segoe UI,10'

$StatusBox                       = New-Object system.Windows.Forms.TextBox
$StatusBox.multiline             = $false
$StatusBox.width                 = 283
$StatusBox.height                = 20
$StatusBox.location              = New-Object System.Drawing.Point(70,255)
$StatusBox.Font                  = 'Microsoft Sans Serif,10'
$StatusBox.MultiLine = $True 


$ToolTip1.SetToolTip($PWSID,'Enter PWS ID (e.g. L19XXX)')
$ToolTip2.SetToolTip($PWSName,'Enter the name of the contract here (do not include the PWS ID)')
$WinForm.controls.AddRange(@($PWSID,$PWSIDText,$PWSNameText,$CreateButton,$PWSName,$Label2,$Label4,$Label1,$Label3,$ADGroup,$Label5,$Picturebox))

$CreateButton.Add_Enter({  })

$PWSIDText.add_TextChanged({IsThereText})
$PWSNameText.add_TextChanged({IsThereText})

$ProgressPreference = 'SilentlyContinue' ##Silently Import Module
Import-Module ActiveDirectory 

#Function to make sure an empty box cannot be submitted
function IsThereText
{
	if ($PWSNameText.Text.Length -and $PWSIDText.Text.Length -ne 0)
	{
		$CreateButton.Enabled = $true
	}
	else
	{
		$CreateButton.Enabled = $false
	}
}

$CreateButton.Add_Click({
#Introduction

#InitialVariables

$PWSID = $PWSIDtext.text 
$PWSName = $PWSNametext.text
$CDest = "\\seddonad.com\ProjectWorkspace\Development Projects\$PWSID $PWSName"
$CSource = "\\seddonad.com\ProjectWorkspace\Development Projects\DevelopmentProjectTemplateDEV"



#iCACLS Variables
$SG = "SEDDONAD\PWS - $PWSID"
$SGIT = "SEDDONAD\PWS - IT Administrators"
$SGSYSTEM = "SYSTEM"
$SGDA = "SEDDONAD\Domain Admins"

$strGrant = "/grant"
$strInh = "/inheritance:r"
$strRead = "/grant:r"
$strIcaclsDefaultPath = "\\seddonad.com\ProjectWorkspace\Development Projects\$PWSID $PWSName"
$strIcaclsPrmsTopLvl = ":r"
$strIcaclsPrms1 = ":(CI)(r,x,rd,ra,rea,wd,wa,wea)" #Top Level Folder Permisions
$strIcaclsPrms2 = ":(OI)(IO)(r,x,rd,ra,rea,wd,wa,wea,ad,d)" #Files Only Permission
$strICaclsPrms3 = ":(OI)(CI)(F)" #Default Groups after Inheritence is disabled
$strIcaclsPrms4 = ":(OI)(CI)(IO)(M,DC)" #Create folders but not delete root folder pt1
$strIcaclsPrms5 = ":(RX,WD,AD)" #Create folders but not delete root folder pt2

if ($ADGroup.Checked -eq $true)
{
New-ADGroup -Name "PWS - $PWSID" -GroupCategory Security -GroupScope Universal -Path 'OU=PWS - Development,OU=Project WorkSpace,OU=Security Groups,DC=seddonad,DC=com'
}


#Copy Folder 
robocopy "$CSource" "$CDest" /COPY:DAS /E /NFL /NDL /NJH /NJS

##Top Level Permissions
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath" $strRead "$SG$strIcaclsPrmsTopLvl"')

##Setting Permission 1. Site Particulars

Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\1. Site Particulars" $strGrant "$SG$strIcaclsPrms1" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\1. Site Particulars" $strGrant "$SG$strIcaclsPrms2" /t')

##Set Permissions 2. Photographs##')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\2. Photographs" $strGrant "$SG$strIcaclsPrms1" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\2. Photographs" $strGrant "$SG$strIcaclsPrms2" /t')

##Set Permissions 3. Financial Appraisals##'
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\3. Development Appraisals" $strGrant "$SG$strIcaclsPrms1" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\3. Development Appraisals" $strGrant "$SG$strIcaclsPrms2" /t')

##Set Permissions 4. Survey Reports##
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\4. Survey Reports" $strGrant "$SG$strIcaclsPrms1" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\4. Survey Reports" $strGrant "$SG$strIcaclsPrms2" /t')

##Set Permissions 5. Planning##
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\5. Planning" $strGrant "$SG$strIcaclsPrms1" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\5. Planning" $strGrant "$SG$strIcaclsPrms2" /t')

##Set Permissions 6. Client or JV Details##
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\6. Client or JV Details" $strGrant "$SG$strIcaclsPrms1" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\6. Client or JV Details" $strGrant "$SG$strIcaclsPrms2" /t')

##Set Permissions 7. Employers Agent##
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\7. Employers Agent" $strGrant "$SG$strIcaclsPrms1" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\7. Employers Agent" $strGrant "$SG$strIcaclsPrms2" /t')

##Set Permissions 8. Cost Plan#
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\8. Cost Plan" $strGrant "$SG$strIcaclsPrms1" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\8. Cost Plan" $strGrant "$SG$strIcaclsPrms2" /t')

##Set Permissions 9. Programme##
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\9. Programme" $strGrant "$SG$strIcaclsPrms1" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\9. Programme" $strGrant "$SG$strIcaclsPrms2" /t')

##Set Permissions 10. Design##
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\10. Design" $strGrant "$SG$strIcaclsPrms1" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\10. Design\10.1 Architects" $strGrant "$SG$strIcaclsPrms2" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\10. Design\10.2.Engineers" $strGrant "$SG$strIcaclsPrms2" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\10. Design\10.3 Landscape" $strGrant "$SG$strIcaclsPrms2" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\10. Design\10.4 Highways" $strGrant "$SG$strIcaclsPrms2" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\10. Design\10.5 Drainage" $strGrant "$SG$strIcaclsPrms2" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\10. Design\10.6 Mech & Elec" $strGrant "$SG$strIcaclsPrms2" /t')

##Set Permissions 11. Working Drawings##
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\11. 3D Model Information" $strGrant "$SG$strIcaclsPrms1" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\11. 3D Model Information\11.1 WIP Design Models" $strGrant "$SG$strIcaclsPrms2" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\11. 3D Model Information\11.2 Shared Models" $strGrant "$SG$strIcaclsPrms2" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\11. 3D Model Information\11.3 Federated Models(NWF,NWD)" $strGrant "$SG$strIcaclsPrms2" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\11. 3D Model Information\11.4 Model Comments, Clash Reports" $strGrant "$SG$strIcaclsPrms2" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\11. 3D Model Information\11.5 Model Renders" $strGrant "$SG$strIcaclsPrms2" /t')

##Set Permissions 12.Meetings##
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\12. Meetings" $strGrant "$SG$strIcaclsPrms1" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\12. Meetings" $strGrant "$SG$strIcaclsPrms2" /t')

##Set Permissions 13. Correspondence##
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\13. Correspondence" $strGrant "$SG$strIcaclsPrms1" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\13. Correspondence" $strGrant "$SG$strIcaclsPrms2" /t')

##Set Permissions 14. Legal##
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\14. Legal" $strGrant "$SG$strIcaclsPrms1" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\14. Legal" $strGrant "$SG$strIcaclsPrms2" /t')

##Set Permissions 15. Consultant Appointments##
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\15. Consultant Appointments" $strGrant "$SG$strIcaclsPrms1" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\15. Consultant Appointments" $strGrant "$SG$strIcaclsPrms2" /t')

##Set Permissions 16. Building Regulation##
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\16. Building Regulation" $strGrant "$SG$strIcaclsPrms1" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\16. Building Regulation" $strGrant "$SG$strIcaclsPrms2" /t')

##Set Permissions 17. Local Authority##
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\17. Local Authority" $strGrant "$SG$strIcaclsPrms1" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\17. Local Authority" $strGrant "$SG$strIcaclsPrms2" /t')

##Set Permissions 18. Statutory Services##
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\18. Statutory Services" $strGrant "$SG$strIcaclsPrms1" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\18. Statutory Services" $strGrant "$SG$strIcaclsPrms2" /t')

##Set Permissions 19. Clarifications##
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\19. Clarifications" $strGrant "$SG$strIcaclsPrms1" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\19. Clarifications" $strGrant "$SG$strIcaclsPrms2" /t')

##Set Permissions 20. Bid Management##
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\20. Bid Management" $strGrant "$SG$strIcaclsPrms1" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\20. Bid Management" $strGrant "$SG$strIcaclsPrms2" /t')

##Set Permissions 21. Final Bid##
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\21. Final Bid" $strGrant "$SG$strIcaclsPrms1" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\21. Final Bid" $strGrant "$SG$strIcaclsPrms2" /t')


##Set Permissions 22. Consultant Appointments##
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\22. SHEQ" $strGrant "$SG$strIcaclsPrms1" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\22. SHEQ" $strGrant "$SG$strIcaclsPrms2" /t')

##Set Permissions 22. CVR##
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\23. CVR" $strGrant "$SG$strIcaclsPrms1" /t')
Invoke-Expression -Command ('icacls "$strIcaclsDefaultPath\23. CVR" $strGrant "$SG$strIcaclsPrms2" /t')



[System.Reflection.Assembly]::LoadWithPartialName("Microsoft.VisualBasic")
[Microsoft.VisualBasic.Interaction]::MsgBox('Project Workspace Script Completed!', 'MsgBoxSetForeground,Information', 'Completed!')
})


[void]$WinForm.ShowDialog()